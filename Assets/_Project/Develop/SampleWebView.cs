...
public class SampleWebView : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        {
            Debug.Log(1); // Выводим сообщение в консоль, если ссылка пуста

            yield break;
        }

        webViewObject = Instantiate(new GameObject("WebViewObject"), GameObject.Find("Web").transform).AddComponent<WebViewObject>();
#if UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
        webViewObject.canvas = GameObject.Find("Web");
#endif
        webViewObject.Init(
            cb: (msg) =>
            {
